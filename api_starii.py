...
# Добавляем эндпоинт для получения списка промокодов
@router.get("/admin/promos")
async def get_promos():
    """
    Получение списка всех промокодов (для админ-панели)
    """
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            db.row_factory = aiosqlite.Row
            
            # Получаем все промокоды из БД, используя rowid вместо id
            async with db.execute(
                "SELECT rowid as id, code, amount, is_used, used_by, created_at FROM promo_codes ORDER BY created_at DESC"
            ) as cursor:
                rows = await cursor.fetchall()
                ...
                for row in rows:
                    promos.append({
                        ...
                        "code": row["code"],
                        "amount": row["amount"],
                        "is_active": not row["is_used"],  # инвертируем значение is_used
                        "used_by": row["used_by"],
        ...
        return {"success": False, "error": "Не удалось получить список промокодов"}

# Добавляем эндпоинт для удаления промокода
@router.post("/admin/delete_promo")
async def delete_promo(promo_id: int = Form(...)):
    """
    Удаление промокода администратором
    """
    try:
        async with aiosqlite.connect(DB_PATH) as db:
            # Проверяем, что промокод существует и активен
            db.row_factory = aiosqlite.Row
            async with db.execute(
                "SELECT is_used FROM promo_codes WHERE rowid = ?",  # Используем rowid вместо id
                (promo_id,)
            ) as cursor:
                ...
                if promo["is_used"]:
                    return {"success": False, "error": "Невозможно удалить использованный промокод"}
            
            # Удаляем промокод
            await db.execute(
                "DELETE FROM promo_codes WHERE rowid = ?",  # Используем rowid вместо id
                (promo_id,)
            )
            ...
            await db.execute(
                "INSERT INTO admin_actions (admin_id, action, timestamp) VALUES (?, ?, ?)",
                (0, f"Удален промокод (ID: {promo_id})", datetime.now().isoformat())
            )
